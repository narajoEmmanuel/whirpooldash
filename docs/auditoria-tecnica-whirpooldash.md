...
| Prediction (XGBoost) | Depende de Azure Blob Storage — **tokens SAS expirados (HTTP 403)** |
...
**Confirmación:** `origin` apunta correctamente a tu fork (`narajoEmmanuel/whirpooldash`). `upstream` tiene push deshabilitado (`remote.upstream.pushurl = DISABLED`), lo que protege contra *pushes* accidentales al repositorio original.
...
| `INSTALL.md` | Notas de instalación (PyPI/TensorFlow); algo desactualizado respecto al código actual. |
...
| `utils/` | Formateo de moneda, porcentajes y números. |
...
| Categoría | Paquetes |
|----------|----------|
| Azure | Ningún SDK oficial (`azure-storage-blob`, etc.). Solo URLs HTTP con tokens SAS. |
...
| `services/run_model.py` | URLs de Azure Blob Storage con token SAS **hardcodeado** (independiente de variables de entorno) |
...
| Predicción XGBoost (UI) | **Azure Blob Storage** obligatorio |
| Price Calculator LSTM | **Azure Blob Storage** (modelos por partner) |
...
| `config.py` | 28-30 | Token SAS de contenedor en Azure Blob Storage | **Alto** — token lectura embebido en repo | Revocar SAS, usar env var / Key Vault, limpiar historial Git |
...
- tokens SAS vigentes (los embebidos están expirados o expuestos)
...
La aplicación **no detiene el arranque**, pero pierde credibilidad analítica: KPIs en cero, Market Performance vacío y selectores de predicción empeorados.
...
|--------|-------------------|----------------------|
| **403 Forbidden** | SAS expirado, revocado o permisos insuficientes | XGB blobs en `run_model.py` (expiran en enero de 2026) |
...
### 10.4 Tokens SAS, almacenamiento propio y estrategia segura

#### Qué es un token SAS en este proyecto
...
| **Render / Railway** | Media–Alta | Variables de entorno; posible servicio separado para FastAPI |
...
|-----------|-------|-----------|
| Seguridad | Tokens SAS y connection strings en código fuente e historial Git | Alta |
...
| ML | Tokens SAS expirados en `run_model.py` — predicción rota | Alta |
...
1. **No commitear** valores reales de tokens SAS/contraseñas.
...
5. El endpoint de Azure Blob Storage responde, pero algunos blobs esperados devuelven 404 y los artefactos XGBoost devuelven 403, por lo que no debe asumirse acceso funcional ni seguro.
